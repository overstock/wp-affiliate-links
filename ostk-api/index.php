...
if($_REQUEST['type'] == 'patterns'){
  echo json_encode($patterns);
  exit();
}

$GLOBALS['developerId'] = FKAJQ7bUdyM;

$atts = $_REQUEST;
unset($atts['PHPSESSID']);
unset($atts['tag']);

echo ostk_generateShortcodeWidgets($atts);
...
function plugin_dir_url(){
	return 'http://localhost/~thoki/ostk-api/';
}
...
function ostk_generateShortcodeWidgets($atts){
  foreach($atts as $key=>$value){
    /* if a value has a "&" like in "Home & Garden" it is sent via url as "Home | Garden" 
    now that it's into php convert it back to a "&" */
